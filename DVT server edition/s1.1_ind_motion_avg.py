...
for ts in range(a):
    ...
    ind_df = pd.DataFrame(columns={'videoname', 'id', 'xlabel', 'ylabel', 'timepoint',
                                   ...
                                   'angular_velocity', 'angular_velocity_non_edge', 'angular_velocity_at_edge',
                                   'track_straightness', 'track_straightness_non_edge', 'track_straightness_at_edge',
                                   'max_angular_velocity', 'max_angular_velocity_non_edge',
                                   'max_angular_velocity_at_edge',
                                   'r_dist', 'r_edge',
                                   'move_proportion_at_edge', 'velocity_at_edge',
                                   'move_proportion_at_center', 'velocity_at_center',
                                   'meander', 'meander_non_edge', 'meander_at_edge',
                                   'max_meander', 'max_meander_non_edge', 'max_meander_at_edge', 'max_velocity_at_edge',
                                   'max_velocity_at_center', 'movelength_at_edge', 'movelength_at_center'
                                   })
    ...
    for i in range(n_inds):
        ...
        if len(tmp_pos)>0:
            ...
            ind_df.loc[i,'track_straightness_non_edge']=np.nan
            ind_df.loc[i,'track_straightness_at_edge']=np.nan
        print(v_name, i,'timepoint',s3,'th dro. velocity and track_straightness finished')
        ...
        print(v_name, i,'timepoint',s3,'th dro. movetime, angular and meander finished')
    # nomove_df.to_csv('1.csv')
        ...
        print(v_name, i,'timepoint',s3,'th dro. searchtime finished')

        pos[move_label]=pos[move_label]*fps
        ind_df.loc[i, 'timepoint'] = s3
    ind_df['movelength_ratio_at_edge'] = ind_df['movelength_at_edge'] / ind_df['total_move_length_threshed']
    whole_ind_df=pd.concat([whole_ind_df,ind_df])
    g_df.loc[ts, 'videoname'] = v_name
    g_df.loc[ts, 'timepoint'] = s3
    g_df.loc[ts, 'movelength_ratio_at_edge'] = np.nanmean(list(ind_df['movelength_ratio_at_edge']))
    g_df.loc[ts, 'search_area_time'] = np.nanmean(list(ind_df['search_area_time']))
    g_df.loc[ts, 'search_area'] = np.nanmean(list(ind_df['search_area']))
    g_df.loc[ts, 'search_area_unit_move'] = np.nanmean(list(ind_df['search_area_unit_move']))
    g_df.loc[ts, 'max_velocity'] = np.nanmean(list(ind_df['max_velocity']))
    g_df.loc[ts, 'max_velocity_threshed'] = np.nanmean(list(ind_df['max_velocity_threshed']))
    g_df.loc[ts, 'total_move_length'] = np.nanmean(list(ind_df['total_move_length']))
    g_df.loc[ts, 'total_move_length_threshed'] = np.nanmean(list(ind_df['total_move_length_threshed']))
    g_df.loc[ts, 'velocity'] = np.nanmean(list(ind_df['velocity']))
    g_df.loc[ts, 'velocity_threshed'] = np.nanmean(list(ind_df['velocity_threshed']))
    g_df.loc[ts, 'velocity_at_edge'] = np.nanmean(list(ind_df['velocity_at_edge']))
    g_df.loc[ts, 'velocity_at_center'] = np.nanmean(list(ind_df['velocity_at_center']))
    g_df.loc[ts, 'move_time'] = np.nanmean(list(ind_df['move_time']))
    g_df.loc[ts, 'move_time_threshed'] = np.nanmean(list(ind_df['move_time_threshed']))
    g_df.loc[ts, 'move_proportion_at_edge'] = np.nanmean(list(ind_df['move_proportion_at_edge']))
    g_df.loc[ts, 'move_proportion_at_center'] = np.nanmean(list(ind_df['move_proportion_at_center']))
    g_df.loc[ts, 'r_dist'] = np.nanmean(list(ind_df['r_dist']))
    g_df.loc[ts, 'r_edge'] = np.nanmean(list(ind_df['r_edge']))
    g_df.loc[ts, 'tracks_num'] = np.nanmean(list(ind_df['tracks_num']))
    g_df.loc[ts, 'tracks_duration'] = np.nanmean(list(ind_df['tracks_duration']))
    g_df.loc[ts, 'tracks_length'] = np.nanmean(list(ind_df['tracks_length']))
    g_df.loc[ts, 'long_stop_num'] = np.nanmean(list(ind_df['long_stop_num']))
    g_df.loc[ts, 'stop_duration'] = np.nanmean(list(ind_df['stop_duration']))
    g_df.loc[ts, 'long_stop_duration'] = np.nanmean(list(ind_df['long_stop_duration']))
    g_df.loc[ts, 'angular_velocity'] = np.nanmean(list(ind_df['angular_velocity']))
    g_df.loc[ts, 'angular_velocity_non_edge'] = np.nanmean(list(ind_df['angular_velocity_non_edge']))
    g_df.loc[ts, 'angular_velocity_at_edge'] = np.nanmean(list(ind_df['angular_velocity_at_edge']))
    g_df.loc[ts, 'track_straightness'] = np.nanmean(list(ind_df['track_straightness']))
    g_df.loc[ts, 'track_straightness_non_edge'] = np.nanmean(list(ind_df['track_straightness_non_edge']))
    g_df.loc[ts, 'track_straightness_at_edge'] = np.nanmean(list(ind_df['track_straightness_at_edge']))
    g_df.loc[ts, 'max_angular_velocity'] = np.nanmean(list(ind_df['max_angular_velocity']))
    g_df.loc[ts, 'max_angular_velocity_non_edge'] = np.nanmean(list(ind_df['max_angular_velocity_non_edge']))
    g_df.loc[ts, 'max_angular_velocity_at_edge'] = np.nanmean(list(ind_df['max_angular_velocity_at_edge']))
    g_df.loc[ts, 'meander'] = np.nanmean(list(ind_df['meander']))
    g_df.loc[ts, 'meander_non_edge'] = np.nanmean(list(ind_df['meander_non_edge']))
    g_df.loc[ts, 'meander_at_edge'] = np.nanmean(list(ind_df['meander_at_edge']))
    g_df.loc[ts, 'max_meander'] = np.nanmean(list(ind_df['max_meander']))
    g_df.loc[ts, 'max_meander_non_edge'] = np.nanmean(list(ind_df['max_meander_non_edge']))
    g_df.loc[ts, 'max_meander_at_edge'] = np.nanmean(list(ind_df['max_meander_at_edge']))

    g_df.loc[ts, 'movelength_at_edge'] = np.nanmean(list(ind_df['movelength_at_edge']))
    g_df.loc[ts, 'movelength_at_center'] = np.nanmean(list(ind_df['movelength_at_center']))
    g_df.loc[ts, 'max_velocity_at_edge'] = np.nanmean(list(ind_df['max_velocity_at_edge']))
    g_df.loc[ts, 'max_velocity_at_center'] = np.nanmean(list(ind_df['max_velocity_at_center']))
...
whole_ind_df_tocsv.to_csv(fileplace_analysis + v_name + '_ind_motion_sliced.csv')
#'max_velocity','velocity','total_move_length','move_time',
g_df_tocsv=g_df[['videoname','timepoint',
               'search_area_time','search_area','search_area_unit_move',
               'velocity_threshed','velocity_at_edge','velocity_at_center',
               'max_velocity_threshed','max_velocity_at_edge','max_velocity_at_center',
               'total_move_length_threshed','movelength_at_edge','movelength_at_center',
...
g_df_tocsv.to_csv(fileplace_analysis + v_name + '_avg_motion_sliced.csv')
