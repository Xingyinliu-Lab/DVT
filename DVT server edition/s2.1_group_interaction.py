...
if n_inds > 1:
    ...
    ind_df = ind_df[['videoname', 'id', 'xlabel', 'ylabel',
                     ...
                     'eccentricity',
                     'dominating']]
    ind_df.to_csv(fileplace_analysis + v_name + '_ind_interaction.csv')
    interaction_df.to_csv(fileplace_analysis + v_name + '_group_interaction_subject_to_subject.csv')
    ...
    g_df = g_df[[
        ...
        'degree',
        'unconnected_social_network_proportion','global_efficiency']]
    g_df.to_csv(fileplace_analysis + v_name + '_avg_interaction.csv')
    moveinfo2 = pos[l_list]
    moveinfo2.to_csv(fileplace_analysis + v_name + '_ind_moveinfo2.csv', index=False)
    #
    ...
    plt.savefig(fileplace_analysis + v_name + '_group_interaction.jpg')
    ...
    plt.tight_layout()
    plt.savefig(fileplace_analysis + v_name + '_social_space.png')
    plt.close()
